...
def generate(request: Any, progress: Optional[Callable[..., Any]] = None) -> Optional[str]:
    """Run a FLUX.2-klein edit on the source image and return the output path."""
    ...
    if progress is not None:
        progress(0, total, "Loading FLUX.2 Klein model")
    ...
    output_path = os.path.join(_output_dir(), f"flux2-edit-{int(time.time() * 1000)}.png")
